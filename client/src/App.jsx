...
import { 
  ...
  Download, Upload, LogOut, X, Eye, Edit3
} from 'lucide-react';
...
export default function App() {
  ...
  const handleRequest = async (endpoint) => { if (!prompt) return; setLoading(true); setLoadingText(endpoint === '/create' ? 'Architecting Layout...' : 'Refining Code...'); const texts = ['Generating Components...', 'Applying Tailwind...', 'Optimizing Assets...', 'Finalizing Build...']; let textIdx = 0; const interval = setInterval(() => { setLoadingText(texts[textIdx % texts.length]); textIdx++; }, 2500); try { const payload = endpoint === '/edit' ? { instruction: prompt, sessionId } : { prompt, sessionId }; const res = await axios.post(API_URL + endpoint, payload); clearInterval(interval); setSiteUrl(res.data.url); setRawCode(res.data.code); saveToHistory(prompt, res.data.code); if (endpoint === '/edit') setPrompt(''); setMobileTab('preview'); } catch (e) { clearInterval(interval); alert("Error: " + e.message); } setLoading(false); };
    ...
    <div className="flex flex-col md:flex-row h-[100dvh] bg-[#050505] text-white font-sans overflow-hidden selection:bg-[#beff50]/30 relative">
      ...
      <div className="hidden md:flex w-80 flex-col border-r border-white/5 bg-[#0a0a0a] z-20 shadow-2xl relative">
        ...
        <div className="flex-1 px-4 flex flex-col gap-4 overflow-y-auto">
          {activeTab === 'generator' ? (
             <GeneratorContent prompt={prompt} setPrompt={setPrompt} loading={loading} siteUrl={siteUrl} handleRequest={handleRequest} deploy={deploy} deployUrl={deployUrl} />
          ) : (
        ...
        <div className="p-4 border-t border-white/5">
          <button onClick={() => setShowAccountMenu(!showAccountMenu)} className={`w-full flex items-center gap-3 p-2 rounded-xl transition-colors group ${showAccountMenu ? 'bg-white/10' : 'hover:bg-white/5'}`}>
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-[#beff50] to-emerald-600 p-[1px]"><div className="w-full h-full rounded-full bg-neutral-900 flex items-center justify-center"><User size={14} className="text-white" /></div></div>
            <div className="flex-1 text-left"><p className="text-xs font-medium text-white truncate max-w-[150px]">{username}</p><p className="text-[10px] text-neutral-500">Free Plan</p></div>
            <Settings size={14} className={`text-neutral-500 transition-transform ${showAccountMenu ? 'rotate-90 text-white' : ''}`} />
          </button>
        </div>
      </div>

      {/* --- RESTORED: THE FLOATING SETTINGS MENU --- */}
      {showAccountMenu && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center md:absolute md:inset-auto md:bottom-20 md:left-4">
           ...
           {/* The Menu Card - Responsive Positioning */}
...
const GeneratorContent = ({ prompt, setPrompt, loading, siteUrl, handleRequest, deploy, deployUrl }) => (<><div className="glass-panel p-4 rounded-2xl relative group transition-all hover:border-[#beff50]/30"><div className="flex justify-between items-center mb-3"><label className="text-xs font-bold text-[#beff50] uppercase tracking-wider flex items-center gap-1"><Command size={10} /> {siteUrl ? "Refine Desgin" : "New Project"}</label>{loading && <Loader2 size={14} className="animate-spin text-neutral-500" />}</div><textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} className="w-full h-32 bg-transparent border-none text-sm text-neutral-200 placeholder-neutral-600 focus:ring-0 resize-none leading-relaxed" placeholder={siteUrl ? "Make buttons larger..." : "A portfolio for..."} /><div className="flex justify-between items-center mt-2 pt-2 border-t border-white/5"><span className="text-[10px] text-neutral-600">Enter to generate</span><button onClick={() => handleRequest(siteUrl ? '/edit' : '/create')} disabled={loading || !prompt} className="bg-white text-black p-2 rounded-lg hover:bg-[#beff50] hover:scale-105 transition-all disabled:opacity-50">{loading ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />}</button></div></div>{siteUrl && <div className="glass-panel p-4 rounded-2xl animate-in fade-in"><div className="flex items-center gap-2 mb-3 text-[#beff50]"><Globe size={14} /><span className="text-xs font-bold uppercase tracking-wide">Deployment</span></div><button onClick={deploy} disabled={loading} className="w-full py-2.5 bg-neutral-800 hover:bg-neutral-700 border border-white/5 text-neutral-200 text-xs font-medium rounded-lg transition-all flex items-center justify-center gap-2">Publish to Surge</button>{deployUrl && <a href={deployUrl} target="_blank" className="mt-3 block text-center text-[10px] text-[#beff50] truncate border-t border-white/5 pt-2 hover:underline">{deployUrl}</a>}</div>}</>);
